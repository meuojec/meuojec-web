...
function beep(type: "ok" | "error") {
  try {
    ...
    o.start();
    setTimeout(() => { o.stop(); ctx.close(); }, type === "ok" ? 120 : 220);
  } catch {}
}

type ActiveEventApi = {
  evento: null | { id: string; id_evento: string | null; nombre: string | null; activated_at: string | null };
  sesion: null | { id: string; evento_id: string; activo: boolean | null; started_at: string | null; ended_at: string | null };
};

type RpcDomingoResult = {
  ok: boolean; code: string; message: string | null;
  asistencia_id?: string | null; nombres?: string | null; apellidos?: string | null;
};
...
export default function EscanearAsistencia() {
  ...
  const refreshActiveSession = async () => {
    try {
      ...
      if (!res.ok || !json.evento) {
        setEventoNombre(null); setEventoId(null); setEventoSesionId(null);
        return;
      }
  ...
  const registrarRut = async (rutRaw: string) => {
    ...
    try {
      ...
      if (error) {
        // Constraint de unicidad llega a veces como error de Supabase en vez de código ALREADY
        const isDuplicate =
          error.message?.toLowerCase().includes("duplicate") ||
          error.message?.toLowerCase().includes("unique constraint");
        if (isDuplicate) {
          setSemaforo("error", "YA REGISTRADO", "Esta persona ya fue registrada hoy");
          beep("error");
        } else {
        ...
        if (!res) {
          ...
          const notFound =
            ...
            res.message?.toLowerCase().includes("no encontrado") ||
            res.code === "NOT_FOUND";
          setSemaforo(
            "error",
            notFound ? "NO REGISTRADO EN LA APP" : "NO REGISTRADO",
            notFound ? rut : (res.message ?? "No se pudo registrar")
          );
          beep("error");
        } else {
      ...
      try {
        await html5Qr.start(
          ...
          async (decodedText: string) => {
            ...
            setTimeout(() => { lastRutRef.current = ""; }, 2500);
          },
          () => {}
        );
      } catch {
  ...
  const openManualPin = () => { setPinError(null); setPinInput(""); setPinModalOpen(true); };
  ...
  const lockManual = () => {
    setManualEnabled(false); setManualRut("");
    setPinModalOpen(false); setPinInput(""); setPinError(null);
  };
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex gap-4 items-end">
            <div className="flex flex-col items-end">
              <div className="text-sm opacity-90 w-full text-left">Hora</div>
        ...
        <div className="rounded-2xl border border-white/20 bg-black/20 p-6 md:p-10">
          <div className="text-center">
            ...
            {statusSubtitle ? (
              ...
              <div className="mt-3 text-lg md:text-2xl opacity-80">Escanea el QR del carnet del miembro</div>
            )}
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/20 bg-black/20 p-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsScanning((v) => !v)} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">
            ...
            {isScanning ? (
              <div className="mt-4 rounded-xl bg-black/30 p-3">
                ...
                <div className="mt-2 text-xs opacity-80">
                  Si no aparece la cámara: permite permisos del navegador. Requiere HTTPS en producción.
                </div>
              </div>
            ) : (
            ...
            <div className="mt-4 flex gap-2">
              <button onClick={refreshActiveSession} className="flex-1 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">
                Actualizar evento/sesión
              </button>
              <button onClick={() => setSemaforo("idle", "Listo para escanear", "")} className="flex-1 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">
                Limpiar pantalla
              </button>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/20 bg-black/20 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {manualEnabled ? (
                <button onClick={lockManual} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">Bloquear manual</button>
              ) : (
                <button onClick={openManualPin} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">Habilitar manual</button>
              )}
            </div>
            {!manualEnabled ? (
              ...
              <>
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") { registrarRut(manualRut); setManualRut(""); } }}
                />
                <button
                  onClick={async () => { await registrarRut(manualRut); setManualRut(""); }}
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-2xl bg-slate-900 border border-white/20 p-5 space-y-4">
              ...
              <input
                ...
                placeholder="PIN"
                onKeyDown={(e) => { if (e.key === "Enter") confirmPin(); if (e.key === "Escape") setPinModalOpen(false); }}
              />
              {pinError && <div className="text-sm text-red-300">{pinError}</div>}
              <div className="flex gap-2">
                <button onClick={() => setPinModalOpen(false)} className="flex-1 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">Cancelar</button>
                <button onClick={confirmPin} className="flex-1 px-4 py-2 rounded-lg bg-white text-black font-semibold hover:bg-gray-100">Confirmar</button>
              </div>
            </div>
          </div>
        )}
