...
const SEXO_OPTIONS = ["Masculino", "Femenino"] as const;
...
const DED_OPTIONS = ["Varones", "Damas", "Jóvenes", "Creyentes", "Aspirantes", "Sin DED"] as const;
const ESTADO_MEMBRESIA_OPTIONS = ["Activo", "En proceso", "Visitante", "Inactivo"] as const;
...
type FormState = {
  rut: string; nombres: string; apellidos: string; sexo: string;
  fecha_nacimiento: string; nacionalidad: string; estado_civil: string;
  fecha_matrimonio: string; anio_matrimonio: string;
  nombre_conyuge: string; numero_hijos: string;
  direccion: string; comuna: string; telefono: string; correo_electronico: string;
  razon_alta: string; fecha_conversion: string; fecha_bautizo: string;
  departamento: string[];
  asiste_antes_otra_iglesia: boolean; nombre_iglesia_anterior: string;
  estado_membresia: string; ded: string;
  profesion_oficio: string; lugar_trabajo_estudio: string;
  nivel_academico: string; prevision: string;
  alergia_medicamento: string; medicacion_permanente: string;
  discapacidad_fisica: boolean;
  tiene_vehiculo: boolean; patente: string; marca_modelo: string;
  observaciones: string; foto: File | null;
};
...
const inputCls =
  "mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-white/20";
  ...
  return (
    <div className="space-y-1" ref={ref}>
      ...
      <button
        type="button"
        onClick={() => setOpen((s) => !s)}
        className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-left text-sm hover:bg-white/5"
      >
        <div className="flex items-center justify-between gap-3">
          <span className={label2 ? "text-white/90" : "text-white/50"}>
            {label2 || placeholder}
          </span>
      ...
      {open && (
        <div className="mt-1 rounded-xl border border-white/10 bg-black/70 backdrop-blur p-2 max-h-[260px] overflow-auto z-30 relative">
          <div className="flex items-center justify-between gap-2 px-2 py-1">
            <div className="text-xs text-white/50">Selecciona uno o varios</div>
            {value.length > 0 && (
              <button
                type="button"
                onClick={() => onChange([])}
...
export default function MemberWizardForm() {
  ...
  const [rutWarning, setRutWarning] = useState("");
  ...
  const [f, setF] = useState<FormState>({
    ...
    fecha_matrimonio: "", anio_matrimonio: "", nombre_conyuge: "", numero_hijos: "",
    direccion: "", comuna: "", telefono: "", correo_electronico: "",
    razon_alta: "", fecha_conversion: "", fecha_bautizo: "",
    departamento: [],
    asiste_antes_otra_iglesia: false, nombre_iglesia_anterior: "",
    estado_membresia: "", ded: "",
    profesion_oficio: "", lugar_trabajo_estudio: "", nivel_academico: "", prevision: "",
    alergia_medicamento: "", medicacion_permanente: "", discapacidad_fisica: false,
    tiene_vehiculo: false, patente: "", marca_modelo: "",
    observaciones: "", foto: null,
  });

  const showFamilia = useMemo(
    () => (f.estado_civil || "").toLowerCase().includes("cas"),
    [f.estado_civil]
  );
  ...
  // ✅ Verificar RUT en BD al salir del campo
  const checkRutExists = useCallback(async (rutRaw: string) => {
    ...
    try {
      ...
      const { data } = await supabase
        ...
        .maybeSingle();
      if (data) {
        setRutWarning("⚠️ Este RUT ya está registrado en el sistema.");
      }
    } catch {
      // silencioso
    } finally {
  ...
  function validateStep(key: StepKey) {
    ...
    if (key === "vehiculo") {
      if (showVehiculoFields && !f.patente.trim())
        errors.patente = "Patente requerida si tiene vehículo.";
    }
    return errors;
  }
  ...
  const stepErrors = useMemo(
    ...
    [step.key, f, showIglesiaAnterior, showVehiculoFields, rutWarning]
  );

  function next() {
    setGlobalErr(""); setOkMsg("");
  ...
  async function submitAll() {
    ...
    for (const s of steps) Object.assign(allErrors, validateStep(s.key));
    ...
    try {
      ...
      fd.set("departamento", f.departamento.join(", "));
      ...
      if (!res.ok) { setGlobalErr(json?.error || "Error guardando el miembro."); return; }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-white/60">Wizard paso a paso para registrar miembros.</p>
        </div>
        <button
          type="button"
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.key} className={cls(
                ...
                : done ? "border-white/10 bg-black/30 text-white/80"
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-5">
        ...
        {step.key === "basico" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* RUT */}
              <div>
                <label className="text-sm text-white/70">RUT<RequiredBadge /></label>
                <input
                  value={f.rut}
                  onChange={(e) => { setField("rut", e.target.value); setRutWarning(""); }}
                  onBlur={() => {
                    setTouched((p) => ({ ...p, rut: true }));
                    checkRutExists(f.rut);
                  }}
                  placeholder="12345678-9 (sin puntos)"
                  className={cls(inputCls, rutWarning ? "border-amber-500/60" : "")}
                />
                {checkingRut && <div className="mt-1 text-xs text-white/50">Verificando RUT...</div>}
                {rutWarning && !checkingRut && (
                  <div className="mt-1 text-xs text-amber-400 font-medium">{rutWarning}</div>
                )}
                <FieldError name="rut" />
              </div>
              ...
              {/* Sexo */}
              <div>
                <label className="text-sm text-white/70">Sexo<RequiredBadge /></label>
                <select
                  value={f.sexo}
                ...
                >
                  ...
                  {SEXO_OPTIONS.map((x) => <option key={x} value={x}>{x}</option>)}
                </select>
              ...
              {/* Nombres */}
              <div>
                <label className="text-sm text-white/70">Nombres<RequiredBadge /></label>
                <input
                  value={f.nombres}
              ...
              {/* Apellidos */}
              <div>
                <label className="text-sm text-white/70">Apellidos<RequiredBadge /></label>
                <input
                  value={f.apellidos}
              ...
              {/* Fecha nacimiento */}
              <div>
                <label className="text-sm text-white/70">Fecha de nacimiento<RequiredBadge /></label>
                <input
                  type="date"
            ...
            <div className="rounded-xl border border-white/10 bg-black/20 p-4 h-fit">
              <div className="text-sm font-semibold text-white text-center mb-3">Foto de identidad</div>
              <div className="rounded-xl border border-white/10 bg-black/30 aspect-[4/5] flex items-center justify-center text-white/50 text-sm overflow-hidden">
                {f.foto ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={URL.createObjectURL(f.foto)} alt="Preview" className="h-full w-full object-cover" />
                ) : "Sin foto"}
              </div>
              <div className="mt-3 text-center space-y-2">
                <label className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs hover:bg-white/10 cursor-pointer">
                ...
                {f.foto && (
                  <div>
                    <button type="button" onClick={() => setField("foto", null)}
                      className="text-xs text-white/60 hover:text-white">
                      Quitar foto
                    </button>
                  </div>
                )}
                <div className="text-xs text-white/40">JPG/PNG/WebP. Opcional.</div>
              </div>
            </div>
          </div>
        )}
        ...
        {step.key === "contacto" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                <option value="">—</option>
                {COMUNAS_RM.map((x) => <option key={x} value={x}>{x}</option>)}
              </select>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={f.estado_membresia} onChange={(e) => setField("estado_membresia", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {ESTADO_MEMBRESIA_OPTIONS.map((x) => <option key={x} value={x}>{x}</option>)}
                </select>
              </div>

              <div>
                <label className="text-sm text-white/70">DED</label>
                <select value={f.ded} onChange={(e) => setField("ded", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {DED_OPTIONS.map((x) => <option key={x} value={x === "Sin DED" ? "" : x}>{x}</option>)}
                </select>
              </div>

              <MultiSelectDropdown
                label="Ministerio / Departamento"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={f.razon_alta} onChange={(e) => setField("razon_alta", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {RAZON_ALTA.map((x) => <option key={x} value={x}>{x}</option>)}
                </select>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" value={f.fecha_conversion} onChange={(e) => setField("fecha_conversion", e.target.value)} className={inputCls} />
                </div>
                <div>
                  <label className="text-sm text-white/70">Fecha bautizo</label>
                  <input type="date" value={f.fecha_bautizo} onChange={(e) => setField("fecha_bautizo", e.target.value)} className={inputCls} />
                </div>
              </div>

              <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer">
                <input type="checkbox" checked={f.asiste_antes_otra_iglesia} onChange={(e) => setField("asiste_antes_otra_iglesia", e.target.checked)} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={f.profesion_oficio} onChange={(e) => setField("profesion_oficio", e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="text-sm text-white/70">Lugar trabajo / estudio</label>
                <input value={f.lugar_trabajo_estudio} onChange={(e) => setField("lugar_trabajo_estudio", e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="text-sm text-white/70">Nivel académico</label>
                <select value={f.nivel_academico} onChange={(e) => setField("nivel_academico", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {NIVEL_ACADEMICO.map((x) => <option key={x} value={x}>{x}</option>)}
                </select>
              </div>
              <div>
                <label className="text-sm text-white/70">Previsión de salud</label>
                <select value={f.prevision} onChange={(e) => setField("prevision", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {PREVISION.map((x) => <option key={x} value={x}>{x}</option>)}
                </select>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={f.alergia_medicamento} onChange={(e) => setField("alergia_medicamento", e.target.value)} placeholder="No / ¿Cuál?" className={inputCls} />
              </div>
              <div>
                <label className="text-sm text-white/70">Medicación permanente</label>
                <input value={f.medicacion_permanente} onChange={(e) => setField("medicacion_permanente", e.target.value)} placeholder="No / ¿Cuál?" className={inputCls} />
              </div>
              <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer">
                <input type="checkbox" checked={f.discapacidad_fisica} onChange={(e) => setField("discapacidad_fisica", e.target.checked)} />
                Discapacidad física
              </label>
            </div>
          </div>
        ...
        {step.key === "vehiculo" && (
          <div className="space-y-4">
            <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer">
              <input type="checkbox" checked={f.tiene_vehiculo} onChange={(e) => setField("tiene_vehiculo", e.target.checked)} />
              Tiene vehículo
            </label>
            {showVehiculoFields && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={f.marca_modelo} onChange={(e) => setField("marca_modelo", e.target.value)} placeholder="Ej: Hyundai Creta" className={inputCls} />
                </div>
              </div>
            )}
          </div>
        ...
        {step.key === "notas" && (
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-white/10 bg-black/20 p-4">
              ...
              <div className="text-sm text-white/70 grid grid-cols-1 md:grid-cols-2 gap-2">
                <div><span className="text-white/50">RUT:</span> {normalizeRut(f.rut) || "—"}</div>
                ...
                <div><span className="text-white/50">Nacionalidad:</span> {f.nacionalidad || "—"}</div>
                <div><span className="text-white/50">Estado civil:</span> {f.estado_civil || "—"}</div>
                <div><span className="text-white/50">DED:</span> {f.ded || "—"}</div>
                <div><span className="text-white/50">Estado membresía:</span> {f.estado_membresia || "—"}</div>
                <div><span className="text-white/50">Departamentos:</span> {f.departamento.join(", ") || "—"}</div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cls(
                ...
                stepIdx === 0 || submitting ? "bg-white/5 text-white/30 cursor-not-allowed" : "bg-black/30 text-white/80 hover:bg-white/5"
              )}
            >← Atrás</button>
